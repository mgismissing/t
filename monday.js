...
function MondayGetItemCountTextFilter(boardId, textFilter) {
    // Define your API key and board ID
    const apiKey = 'eyJhbGciOiJIUzI1NiJ9.eyJ0aWQiOjM0NjYyNDg5MCwiYWFpIjoxMSwidWlkIjo1ODg4MTMwMiwiaWFkIjoiMjAyNC0wNC0xM1QyMjoxNDozNi4wMDBaIiwicGVyIjoibWU6d3JpdGUiLCJhY3RpZCI6MjI2ODUzNTgsInJnbiI6ImV1YzEifQ.vVl92pM2Tf7-g4h2n9gSm4pXxbdUS-3r5lM_YOgNhz4';

    // Define the endpoint for querying items in the board
    ...
    query {
        boards (ids: ${boardId}) {
        items_by_column_values (column_values: {text: "${textFilter}"}, limit: 1) {
            count
        }
        }
    }
    ...
    .then(data => {
        // Handle the API response data
        const count = data.data.boards[0].items_by_column_values[0].count;
        return count;
    })
    .catch(error => {
        // Handle any errors that occurred during the fetch
